...
def calculate_spec_market_ratio(vehicle: dict, all_vehicles: List[dict]) -> float:
    ...
    Uses caching to avoid redundant calculations for the same spec within a group.
    """
    ...
    # Create a cache key
    cache_key = f"{vehicle_make}_{vehicle_model}_spec_{vehicle_spec}"
    
    # Check if we've already calculated this ratio
    if cache_key in _spec_ratio_cache:
        return _spec_ratio_cache[cache_key]
    ...
    # Cache the result
    _spec_ratio_cache[cache_key] = price_ratio
    
    return price_ratio


# Cache for category ratios to avoid redundant calculations
_category_ratio_cache = {}
...
def calculate_dynamic_category_ratio(vehicle: dict, all_vehicles: List[dict], category_field: str) -> float:
    ...
    Uses caching to avoid redundant calculations for the same category within a group.
    ...
    # Create a cache key combining make, model, category field and value
    cache_key = f"{vehicle_make}_{vehicle_model}_{category_field}_{vehicle_category}"
    
    # Check if we've already calculated this ratio
    if cache_key in _category_ratio_cache:
        return _category_ratio_cache[cache_key]
    ...
    _category_ratio_cache[cache_key] = price_ratio
...
def extract_regression_features(vehicle: dict, all_vehicles: List[dict] = None) -> List[float]:
    ...
    Optimized with caching to avoid redundant calculations for the same vehicle.
    ...
    """
    # Create a unique identifier for this vehicle
    vehicle_id = vehicle.get('deal_id')
    
    # If we've already extracted features for this vehicle, return the cached result
    if vehicle_id and vehicle_id in _feature_cache:
        return _feature_cache[vehicle_id]
...
def estimate_enhanced_market_value(car: dict, similar_cars: List[dict], cfg: Dict = CONFIG, silent: bool = False) -> float:
    """
    ...
    base_analysis = dynamic_price_analysis(car, similar_cars, cfg, silent=silent)
    estimated_value = float(base_analysis[0])  # First element is estimated_market_value
    confidence = float(base_analysis[1])  # Second element is confidence
    
    if estimated_value <= 0 and not silent:
...
def reset_analysis_caches():
    """
    Reset all caches used for analysis.
    Call this before analyzing a new group to ensure fresh calculations.
    """
    global _category_ratio_cache, _spec_ratio_cache, _feature_cache, _make_model_cache
    _category_ratio_cache = {}
    _spec_ratio_cache = {}
    _feature_cache = {}
    _make_model_cache = {}

def enhanced_analyse_listings(listings: List[dict], cfg: Dict = CONFIG, verbose: bool = False) -> None:
    """
    ...
    # Reset all caches to ensure fresh analysis for this group
    ...
    # Step 1: Create a single regression model for the entire group
    # This avoids creating a new model for each car
    group_regression_model = train_group_regression_model(listings, cfg, silent=True)
    
    # Step 2: Use the pre-trained model to analyze each car
    for i, listing in enumerate(listings):
        if verbose and i % 20 == 0:
            print(f"   Processing listing {i+1}/{original_count}...")
        
        # Use the pre-trained model for analysis
        enhanced_value, confidence = predict_with_group_model(listing, group_regression_model, silent=True)
        ...
        listing['enhanced_market_confidence'] = confidence
    ...
    # No detailed output here - moved to scrape_grouping.py
...
def convert_network_request_to_listing(network_car: dict) -> dict:
    ...
    Maps the streamlined network request data to analysis-ready format.
    Only includes fields essential for analysis and minimal required metadata.
    """
...
# ────────────────────────────────────────────────────────────
# Optimized regression model functions for group analysis
# ────────────────────────────────────────────────────────────

def train_group_regression_model(listings: List[dict], cfg: Dict = None, silent: bool = False) -> Dict:
    """
    Train a single regression model for an entire make/model group.
    This avoids redundantly training the same model for each car.
    ...
    Returns:
        dict: {
            ...
            'y': target values,
            'confidence': R² score,
            'percentile_99': 99th percentile value for capping
        }
    """
    if cfg is None:
        cfg = CONFIG
    ...
    if not silent:
        print(f"   🧠 Training regression model for {len(listings)} vehicles...")
    
    # Prepare training data
    X = []  # Features
    y = []  # Prices
    
    # Step 1: Extract features for all valid listings
    for v in listings:
        price = v.get('price_numeric', 0)
        if price > 500:  # Valid price threshold
            features = extract_regression_features(v, listings)
            X.append(features)
            y.append(price)
    ...
    # Handle NaN values by using an imputer
    from sklearn.impute import SimpleImputer
    ...
    # Calculate 99th percentile for capping predictions
    percentile_99 = np.percentile(y, 99) if len(y) > 0 else 0
    
    if not silent:
        print(f"   📊 Model trained: R²={confidence:.3f}, n={len(X)}")
    
    # Return the trained model and related components
    return {
        'model': model,
        'scaler': scaler,
        'X': X,
        ...
        'sample_size': len(X),
        'all_listings': listings  # Include all listings for feature extraction
    }

def predict_with_group_model(vehicle: dict, group_model: Dict, silent: bool = False) -> Tuple[float, float]:
    """
    Use a pre-trained group regression model to predict the market value for a vehicle.
    
    Args:
        vehicle: Vehicle to predict market value for
        group_model: Pre-trained regression model and components
        silent: Whether to suppress output
        
    Returns:
        Tuple[float, float]: (estimated_value, confidence)
    """
    # If no valid model, return 0
    if not group_model or not group_model['model']:
        return 0.0, 0.0
    
    # Extract model components
    model = group_model['model']
    scaler = group_model['scaler']
    confidence = group_model['confidence']
    percentile_99 = group_model['percentile_99']
    
    # Extract features for the target vehicle
    # Note: We still use all listings for ratio calculations
    features = extract_regression_features(vehicle, group_model.get('all_listings', []))
    
    # Check for NaN values and replace with means
    # Also ensure we have the expected number of features
    expected_features_count = 7  # Based on extract_regression_features implementation
    
    # Ensure we have the right number of features
    if len(features) != expected_features_count:
        # If missing features, pad with zeros
        features = features + [0] * (expected_features_count - len(features))
        # If too many features, truncate
        features = features[:expected_features_count]
    
    # Replace any NaN values with zeros
    features = [0 if val is None or (isinstance(val, float) and np.isnan(val)) else val 
                for val in features]
    
    # Scale features and predict
    features_scaled = scaler.transform([features])
    estimated_value = model.predict(features_scaled)[0]
    
    # Ensure reasonable bounds
    estimated_value = max(estimated_value, 500)  # Minimum value
    estimated_value = min(estimated_value, percentile_99)  # Cap at 99th percentile
    
    current_price = float(vehicle.get('price_numeric', vehicle.get('price', 0)))
    profit_potential = estimated_value - current_price
    
    if not silent:
        print(f"   💰 Market value: £{estimated_value:,.0f} (R²={confidence:.3f})")
...
def dynamic_price_analysis(listing: dict, similar_cars: List[dict], cfg: Dict = CONFIG, silent: bool = False) -> Tuple[float, float, float]:
    """
    Perform price analysis for a vehicle listing using regression-based market value estimation.
    Returns: (estimated_market_value, confidence, profit_potential)
    """
    # Use regression-based estimation as the primary method
    result = estimate_market_value_with_regression_full(listing, similar_cars, cfg, silent=silent)
    ...
    return estimated_value, confidence, profit_potential

# ────────────────────────────────────────────────────────────
# Market Value Estimation Functions
# ────────────────────────────────────────────────────────────

# Cache for filtered vehicles by make/model
_make_model_cache = {}
...
def filter_same_make_model_vehicles(target_vehicle: dict, all_vehicles: List[dict]) -> List[dict]:
    ...
    # Create a cache key for this make/model combination
    cache_key = f"{target_make}_{target_model}"
    
    # Check if we've already filtered for this make/model
    if cache_key in _make_model_cache:
        return _make_model_cache[cache_key]
    ...
    # Cache the result
    _make_model_cache[cache_key] = same_make_model
    
    return same_make_model


def estimate_market_value_with_regression_full(vehicle: dict, all_vehicles: List[dict], cfg: Dict = CONFIG, silent: bool = False) -> Dict:
    """
    Estimate market value using regression-based approach and return full result dictionary.
    """
    ...
    result = calculate_regression_market_value(vehicle, same_make_model_vehicles, cfg, silent=silent)
...
def estimate_market_value_with_regression(vehicle: dict, all_vehicles: List[dict], cfg: Dict = CONFIG, silent: bool = False) -> float:
    """
